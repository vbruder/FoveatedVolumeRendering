...
Q_DECLARE_METATYPE(LBGStippling::PointMappingFunction)

SettingsWidget::SettingsWidget(StippleViewer* stippleViewer,
                               QWidget* parent,
                               LBGStippling::Params& params)
    : QWidget(parent), m_stippleViewer(stippleViewer), m_params(params) {
    ...
    QLabel* pointSizeScalingsLabel = new QLabel("Point Size Mapping:", this);
    QComboBox* pointSizeScalings = new QComboBox(this);
    {
        using pmf = LBGStippling::PointMappingFunction;
        pointSizeScalings->addItem("Linear", QVariant::fromValue(pmf::LINEAR));
        pointSizeScalings->addItem("Square root", QVariant::fromValue(pmf::SQUAREROOT));
        pointSizeScalings->addItem("Exponential", QVariant::fromValue(pmf::EXPONENTIAL));
        pointSizeScalings->addItem("Square", QVariant::fromValue(pmf::SQUARE));
        pointSizeScalings->setCurrentIndex(params.mapping);
        pointSizeScalings->setToolTip("How the minimal and maximal point size is mapped.");
        connect(pointSizeScalings, QOverload<int>::of(&QComboBox::currentIndexChanged),
                [this, pointSizeScalings](int index) {
                    m_params.mapping = pointSizeScalings->itemData(index).value<pmf>();
                });
    }
    ...
    pointGroupLayout->addWidget(pointSizeScalingsLabel, 5, 0);
    pointGroupLayout->addWidget(pointSizeScalings, 5, 1);
    ...
    QGroupBox* jsonBox = new QGroupBox("Load params from JSON:", this);
    QVBoxLayout* jsonBoxLayout = new QVBoxLayout(jsonBox);
    jsonBox->setLayout(jsonBoxLayout);
    QPushButton* jsonButton = new QPushButton("Select", this);
    jsonBoxLayout->addWidget(jsonButton);
    connect(jsonButton, &QPushButton::pressed, [=]() {
        QString path = QFileDialog::getOpenFileName(this, tr("Open Settings File"), QString(),
                                                    tr("Settings (*.json)"));

        if (path.isEmpty())
            return;

        m_params.loadParametersJSON(path);
        ...
        spinHysterresis->setValue(m_params.hysteresis);
        spinHysteresisDelta->setValue(m_params.hysteresisDelta);
        pointSizeScalings->setCurrentIndex(m_params.mapping);
    });

    layout->addWidget(jsonBox);
    ...
    connect(m_saveSVG, &QPushButton::pressed, [this]() {
        ...
        m_params.saveParametersJSON(path);
    });
    ...
    connect(startButton, &QPushButton::released, [fileButton, jsonButton]() {
    ...
    connect(m_stippleViewer, &StippleViewer::finished, [fileButton, jsonButton, progressBar]() {
